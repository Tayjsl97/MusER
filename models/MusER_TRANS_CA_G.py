...
class VQ_VAE(nn.Module):
    def __init__(self,N,H,encoder_width,decoder_width,num_embeddings,embedding_dim,dropout,activate,
        ...
        self.decoder_cross_linear = nn.Linear(embedding_dim, decoder_width)
        self.transformer_decoder = TransformerDecoderBuilder.from_kwargs(
            ...
            activation=activate,
            dropout=dropout,
            self_attention_type=decoder_attention_type,
    ...
    def decoder(self, input_x, VQ_embedding):
        ...
        VQ_embedding = self.decoder_cross_linear(VQ_embedding)
    ...
    def predict_linear(self,predict_fc,output,ele,type_sample,flag):
        if flag=="type":
    ...
    def compute_reg_loss(self,input,ele_laten_var):
        ...
        for i in range(7):
            ...
            # print(input_temp.shape,ele_temp.shape,input_temp[0].shape,ele_temp[0].shape)
    ...
    def forward(self,input_x,input_y):
        ...
        type = self.predict_linear(self.predict_type, output, quantized[:, :, 48:64], input_y, "type")
        tempo = self.predict_linear(self.predict_tempo, output, quantized[:, :, 0:16], input_y,"nontype")
        chord = self.predict_linear(self.predict_chord, output, quantized[:, :, 16:32], input_y,"nontype")
        beat = self.predict_linear(self.predict_bar_beat, output, quantized[:, :, 32:48], input_y,"nontype")
        pitch = self.predict_linear(self.predict_pitch, output, quantized[:, :, 64:80], input_y,"nontype")
        duration = self.predict_linear(self.predict_duration, output, quantized[:, :, 80:96],input_y, "nontype")
        velocity = self.predict_linear(self.predict_velocity, output, quantized[:, :, 96:112],input_y, "nontype")
    ...
    def inference(self,dictionary,quantized,emotion=None):
        ...
        with torch.no_grad():
            ...
            while (True):
                ...
                type = self.predict_linear(self.predict_type, output, quantized_type, None, "type")
                type_sample = sampling(type[:,seq_i,:], p=0.9, is_training=False)
                type_sampling_list[:,seq_i]=type_sample
                tempo = self.predict_linear(self.predict_tempo, output, quantized_tempo, type_sampling_list,"nontype")
                chord = self.predict_linear(self.predict_chord, output, quantized_chord, type_sampling_list,"nontype")
                beat = self.predict_linear(self.predict_bar_beat, output, quantized_beat, type_sampling_list,"nontype")
                pitch = self.predict_linear(self.predict_pitch, output, quantized_pitch, type_sampling_list,"nontype")
                duration = self.predict_linear(self.predict_duration, output, quantized_duration,type_sampling_list, "nontype")
                velocity = self.predict_linear(self.predict_velocity, output, quantized_velocity,type_sampling_list, "nontype")
