...
class VQ_VAE(nn.Module):
    ...
    def subDecoder(self,decoder,predict_fc,input_x,ele,type_sample,flag):
        input_x_emb = self.input2emb(input_x)
        input_x_emb = torch.cat(input_x_emb, dim=-1)
        output = self.input_x_linear_emo(input_x_emb)
    ...
    def forward(self,input_x,input_y):
        ...
        # decode
        type = self.subDecoder(self.type_decoder, self.predict_type, input_x, quantized[:, :, 48:64], input_y, "type")
        tempo = self.subDecoder(self.tempo_decoder, self.predict_tempo, input_x, quantized[:, :, 0:16], input_y,"nontype")
        chord = self.subDecoder(self.chord_decoder, self.predict_chord, input_x, quantized[:, :, 16:32], input_y,"nontype")
        beat = self.subDecoder(self.beat_decoder, self.predict_bar_beat, input_x, quantized[:, :, 32:48], input_y,"nontype")
        pitch = self.subDecoder(self.pitch_decoder, self.predict_pitch, input_x, quantized[:, :, 64:80], input_y,"nontype")
        duration = self.subDecoder(self.duration_decoder, self.predict_duration, input_x, quantized[:, :, 80:96],input_y, "nontype")
        velocity = self.subDecoder(self.velocity_decoder, self.predict_velocity, input_x, quantized[:, :, 96:112],input_y, "nontype")
    ...
    def inference(self,dictionary,quantized,emotion=None):
        ...
        with torch.no_grad():
            ...
            seq_i=0
            while (True):
                type = self.subDecoder(self.type_decoder, self.predict_type, input, quantized_type, None, "type")
                type_sample = sampling(type[:,seq_i,:], p=0.9, is_training=False)
                type_sampling_list[:,seq_i]=type_sample
                tempo = self.subDecoder(self.tempo_decoder, self.predict_tempo, input, quantized_tempo, type_sampling_list,"nontype")
                chord = self.subDecoder(self.chord_decoder, self.predict_chord, input, quantized_chord, type_sampling_list,"nontype")
                beat = self.subDecoder(self.beat_decoder, self.predict_bar_beat, input, quantized_beat, type_sampling_list,"nontype")
                pitch = self.subDecoder(self.pitch_decoder, self.predict_pitch, input, quantized_pitch, type_sampling_list,"nontype")
                duration = self.subDecoder(self.duration_decoder, self.predict_duration, input, quantized_duration,type_sampling_list, "nontype")
                velocity = self.subDecoder(self.velocity_decoder, self.predict_velocity, input, quantized_velocity,type_sampling_list, "nontype")
