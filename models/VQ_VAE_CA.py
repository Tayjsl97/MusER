...
class VQ_VAE(nn.Module):
    ...
    def encoder(self,input_y):
        input_y_emb_initial = self.input2emb(input_y)
        input_y_emb = torch.cat(input_y_emb_initial[:-1], dim=-1)
    ...
    def forward(self,input_x,input_y):
        ...
        return type, tempo, chord, bar_beat, pitch, duration, velocity, loss
    ...
    def inference(self,dictionary,memory,emotion=None):
        ...
        with torch.no_grad():
            ...
            while (True):
                ...
                type_sample, tempo, chord, bar_beat, pitch, duration, velocity = self.predict(
                    output[0].narrow(0, seq_i, 1), None)
